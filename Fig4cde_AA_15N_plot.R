# Fig. 4c-e: 15N enrichment of amino acids (replicate 1), with two-way ANOVA
# (Sym x Germ), Tukey HSD post-hoc, non-parametric alternatives, and
# cumulative pairwise t-tests.
#
# Input : Fig4cde_AA.xlsx
# Output: Fig4cde_AA_delta15N_boxplot.pdf
#         Supplementary_Table_assumption_checks_Fig4.csv
#         Supplementary_Table_ANOVA_Fig4.csv
#         Supplementary_Table_ANOVA_FDR_Fig4.csv
#         Supplementary_Table_TukeyHSD_Fig4.csv
#         Supplementary_Table_NonParametric_Fig4.csv
#         Supplementary_Table_Cumulative_Pairwise_Fig4d.csv
...
df <- read_xlsx("Fig4cde_AA.xlsx")

# ---- Derive factors (symbiosis state, germ presence) and 15N/14N ratio ----
df <- df %>%
  mutate(
...
# Normalize each AA to its matched control mean to obtain Δδ15N (per mil)
normalized_df <- df %>%
  group_by(Animal, AAs) %>%
...
# ---- 1. Sample sizes ----
sample_sizes_fig4 <- normalized_df %>% count(AAs, Animal, name = "n")

# ---- 2. Assumption checks ----
assumption_checks_fig4 <- normalized_df %>%
  group_by(AAs) %>%
  summarize(
    shapiro_p = {
...
write.csv(assumption_checks_fig4,
          "Supplementary_Table_assumption_checks_Fig4.csv", row.names = FALSE)

# ---- 3. Full two-way ANOVA (per amino acid) ----
full_anova_fig4 <- normalized_df %>%
  group_by(AAs) %>%
  reframe({
    mod <- aov(delta_15N ~ Sym * Germ, data = cur_data())
    at  <- summary(mod)[[1]]
...
write.csv(full_anova_fig4, "Supplementary_Table_ANOVA_Fig4.csv", row.names = FALSE)

# ---- 4. FDR correction across amino acids ----
fdr_corrected_fig4 <- full_anova_fig4 %>%
  filter(!is.na(p_value)) %>%
  group_by(Source) %>%
  mutate(p_adjusted = p.adjust(p_value, method = "BH")) %>%
  ungroup()
write.csv(fdr_corrected_fig4, "Supplementary_Table_ANOVA_FDR_Fig4.csv",
          row.names = FALSE)

# ---- 5. Tukey HSD post-hoc ----
posthoc_fig4 <- normalized_df %>%
  group_by(AAs) %>%
  reframe({
...
write.csv(posthoc_fig4, "Supplementary_Table_TukeyHSD_Fig4.csv",
          row.names = FALSE)

cld_fig4 <- normalized_df %>%
  group_by(AAs) %>%
  reframe({
    mod <- aov(delta_15N ~ Animal, data = cur_data())
    overall_p <- summary(mod)[[1]]$`Pr(>F)`[1]
...
write.csv(nonparam_fig4, "Supplementary_Table_NonParametric_Fig4.csv",
          row.names = FALSE)
...
write.csv(cumulative_pairwise,
          "Supplementary_Table_Cumulative_Pairwise_Fig4d.csv", row.names = FALSE)

# ---- 8. Figure ----
anova_labels_fig4 <- normalized_df %>%
  group_by(AAs) %>%
  summarize(
    label = {
      mod <- aov(delta_15N ~ Sym * Germ, data = cur_data())
...
ggsave("Fig4cde_AA_delta15N_boxplot.pdf", plot = p_revised,
