# Fig. 1c-e: Representative 16S phyla boxplots with two-way ANOVA,
# Tukey HSD post-hoc tests and non-parametric alternatives.
#
# Input : Fig1cde_phyla_boxplot.xlsx
# Output: Fig1cde_phyla_boxplot.pdf
#         Supplementary_Table_assumption_checks_Fig1.csv
#         Supplementary_Table_ANOVA_Fig1.csv
#         Supplementary_Table_ANOVA_FDR_Fig1.csv
#         Supplementary_Table_TukeyHSD_Fig1.csv
#         Supplementary_Table_NonParametric_Fig1.csv
...
df <- read_xlsx("Fig1cde_phyla_boxplot.xlsx")

data_long <- df %>%
  pivot_longer(cols = -c(sample, group, tissue, symbiosis),
               names_to = "Order", values_to = "Percentage")

# ---- 1. ANOVA assumption checks (Shapiro-Wilk, Levene) ----
...
write.csv(assumption_checks, "Supplementary_Table_assumption_checks_Fig1.csv",
          row.names = FALSE)
...
write.csv(full_anova_results, "Supplementary_Table_ANOVA_Fig1.csv", row.names = FALSE)
...
pval_summary <- full_anova_results %>%
  ...
  ungroup()
write.csv(pval_summary, "Supplementary_Table_ANOVA_FDR_corrected_Fig1.csv",
          row.names = FALSE)
...
write.csv(posthoc_results, "Supplementary_Table_TukeyHSD_Fig1.csv", row.names = FALSE)

cld_results <- data_long %>%
  group_by(Order) %>%
  reframe({
    mod <- aov(Percentage ~ group, data = cur_data())
    overall_p <- summary(mod)[[1]]$`Pr(>F)`[1]
...
# ---- 5. Non-parametric alternative (Kruskal-Wallis + Dunn's) ----
...
write.csv(nonparam_results, "Supplementary_Table_NonParametric_Fig1.csv",
          row.names = FALSE)

# ---- 6. Sample sizes ----
sample_sizes <- data_long %>%
  filter(!is.na(Percentage)) %>%
  distinct(Order, group, sample) %>%
  count(Order, group, name = "n")
...
p <- ggplot(data_long, aes(x = group, y = Percentage, fill = group)) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.2, alpha = 0.6, size = 1.5) +
  facet_wrap(~Order, scales = "free_y") +
  geom_text(data = anova_labels,
            aes(x = 1, y = max_y * 0.95, label = label),
...
ggsave("Fig1cde_phyla_boxplot.pdf", plot = p,
